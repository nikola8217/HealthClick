import { createAppointment, getByIDAppointment } from '../../test/helpers';
...
describe('Get appointment by id', () => {
    ...
    it('return a 401 if user is not authenticated', async () => {
        await getByIDAppointment(appointmentId, false).expect(401);
    });

    it('returns a 400 if appointment does not exist', async () => {
        await getByIDAppointment(appointmentId, true).expect(400);
    });

    it('returns a 200 if appointment is found', async () => {
        ...
        await getByIDAppointment(appointment.body._id, true).expect(200);
    });
});